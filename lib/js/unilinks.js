...
pathprefix = 'https://cdn.jsdelivr.net/gh/unilinks/data@';
filepath = "/"
...
function createPanel(obj) {
  ...
  for (var i = 0; i < obj.entries.length; i++) {
    ...
    if (obj.entries[i].name == "") continue;

    var a = document.createElement('a');
    a.className = 'list-group-item';
    a.setAttribute('target', 'blank');
    a.setAttribute('href', obj.entries[i].url);
    if (obj.entries[i].hasOwnProperty("modul-id") && obj.entries[i]["modul-id"] != "") {
      a.appendChild(document.createTextNode(obj.entries[i].name + " (" + obj.entries[i]["modul-id"] + ")"));
    }
    else {
...
function createTabs(list) {
  ...
  ga.setAttribute('href', 'https://github.com/unilinks');
...
function loadRefresh(id) {
  ...
  if (files != false) {
    ...
    for (var j = 0; j < files.length; j++) {
      var filePath = pathprefix + tag + filepath + files[j];
      var data = JSON.parse(getFile(filePath));
...
function findFiles(id) {
  ...
  if (json != false) {
    ...
    for (var i = 0; i < index.length; i++) {
      if (index[i].id == id) return index[i].files;
    }
  }
  return false;
}
...
function getTag() {
  var data = getFile('https://api.github.com/repos/unilinks/data/tags');
