...
function loadRefresh(id) {
    ...
    sections.html('<div class="list-group d-sm-none d-none d-md-none d-lg-block position-fixed shadow my-3"></div>');
    sections = $('#sections div');

    $(studiengaenge[id]).each(function (id, json_file) {
        $.getJSON(github_raw_url + tag + "/data/" + json_file).done(function (data) {
            $(data).each(function (id, item) {
                ...
                $(item.entries).each(function (id, entry) {
                    if (entry.url !== "") {
                        let entry_link = '<a href="' + entry.url + '" class="list-group-item list-group-item-action" target="_blank">' + entry.name;
                        if (typeof entry["modul-id"] !== "undefined")
                            entry_link += '<small class="text-secondary float-right">' + entry["modul-id"] + '</small>';
                        entry_link += '</a>';
                        panel.append(entry_link);
                    } else {
                        panel.append('<span class="list-group-item list-group-item-action disabled">' + entry.name + '</span>');
                    }
                });
            });
