...
let studiengaenge = [];
let tag, links_div, section_links, progressbar;
...
function setProgressbarWidth(percent) {
    ...
    if (percent < 100)
        ...
        progressbar.delay(300).fadeOut("slow");
}

function err_msg(text) {
    $.snackbar({content: "Ein Fehler ist aufgetreten (" + text + ")"});
}
    ...
    $.each(file_contents, function (id, sections) {
        $.each(sections, function (id, section) {
            ...
            $.each(section["entries"], function (id, entry) {
                ...
                if (entry.url !== "")
                    item_div = '<a class="list-group-item" href="' + entry.url + '" target="_blank">';
                else
                    item_div = '<a class="list-group-item disabled" href="./mithelfen.html" target="_blank">';

                item_div += '<div class="bmd-list-group-col">';
                item_div += '<p class="list-group-item-heading h6">' + entry.name + '</p>';

                if (typeof entry["modul-name"] !== "undefined" && entry["modul-name"] !== "") {
                    item_div += '<p class="list-group-item-text">Modul: ' + entry["modul-name"];
                    if (typeof entry["modul-id"] !== "undefined" && entry["modul-id"] !== "")
                        item_div += ' (' + entry["modul-id"] + ')';
                    item_div += '</p>';
            ...
            section_links.append('<a class="list-group-item list-group-item-action" href="#sec-' + (i++) + '" ' +
                'style="hyphens: auto"\n>' + section.title + '</a>');
